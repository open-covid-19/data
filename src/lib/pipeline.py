...
class DataPipeline:
    ...
    def filter(
            self, data: DataFrame, filter_func: Callable[[Any], bool], **filter_opts) -> DataFrame:
    ...
    def run(self, aux: List[DataFrame], fetch_opts: Dict[str, Any] = None,
            parse_opts: Dict[str, Any] = None, merge_opts: Dict[str, Any] = None,
            filter_func: Callable[[Any], bool] = None, filter_opts: Dict[str, Any] = None,
            patch: DataFrame = None, patch_opts: Dict[str, Any] = None) -> DataFrame:
    ...
    * TODO: finish this list
    ...
    def merge(self, record: Dict[str, Any], aux: List[DataFrame], **merge_opts) -> str:
        ...
        if match_string is not None:
            ...
            aux_regex = aux['match_regex'][aux_mask].apply(lambda x: re.compile(x, re.IGNORECASE))
...
class PipelineChain:
    ...
    def run(
            self, thread_count: int = cpu_count(), group_keys: List[str] = None,
            **pipeline_opts) -> DataFrame:
        '''
        ...
        func_iter = [partial(pipeline.run, aux, **{**opts, **pipeline_opts})
                     for pipeline, opts in self.pipelines]
        pipeline_results = tqdm(Pool(4).imap(PipelineChain._run_wrapper, func_iter),
                                total=len(func_iter), desc=self.__class__.__name__)
